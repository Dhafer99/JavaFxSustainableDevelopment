...
public class Controller implements Initializable {
    ...
    private ObservableList<Association> originalDonList;
private ObservableList<Association> filteredDonList;
    @FXML
...
mediaPlayer.play();*/
    try {
        ...
        List<Association> personnes = s.afficher();
        
        // sort the list by position in list in descending order
        Collections.sort(personnes, new Comparator<Association>() {
            @Override
            public int compare(Association d1, Association d2) {
        ...
        for (Association d : personnes) {
            ...
            fxmlLoader.setLocation(getClass().getResource("/Views/song.fxml"));
...
private void filterDonList(String query) throws SQLException {
    ...
    for (Association d : originalDonList) {
...
private void updateUI(ObservableList<Association> donList) throws SQLException, IOException {
    // Clear the container before populating it
    favoriteContainer.getChildren().clear();
  
    for (Association d : donList) {
        FXMLLoader fxmlLoader = new FXMLLoader();
        fxmlLoader.setLocation(getClass().getResource("/Views/song.fxml"));
...
// display the sorted Don objects in the CardView
 favoriteContainer.getChildren().clear();
    ...
    private void back(MouseEvent event) throws IOException {
        ...
        backBtn.getScene().setRoot(root2); 
    }
