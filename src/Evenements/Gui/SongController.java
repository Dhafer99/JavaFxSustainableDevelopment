...
import Evenements.Entities.Participation;
import Evenements.Entities.User;
import static Evenements.Gui.AddEventController.ACCOUNT_SID;
import static Evenements.Gui.AddEventController.AUTH_TOKEN;
    ...
    controller.receiveObject(d);
     ...
     public void receiveObject(Evenement d) {
        ...
        Evenement=d;
         ...
         try {
             if(!sv.checkEventUser(d.getId(),projetpidd.ProjetPiDD.user.getId()))
             {
...
@FXML
    private void test(ActionEvent event) throws SQLException {
       ...
       Evenement e = new Evenement();
              Connection cnx = MyDB.getInstance().getCnx();
PreparedStatement ps = cnx.prepareStatement("SELECT nom FROM user WHERE id ="+ projetpidd.ProjetPiDD.user.getId());
...
if(rs1.next()){
    ...
    if(Events.containsKey(d.getId()) && Events.containsValue(projetpidd.ProjetPiDD.user.getId()))
    {
...
public void pdf() throws FileNotFoundException, DocumentException, WriterException, IOException, SQLException {
  ...
  Connection cnx = MyDB.getInstance().getCnx();
PreparedStatement ps = cnx.prepareStatement("SELECT nom FROM user WHERE id = 120");
...
imageE.setAlignment(Element.ALIGN_LEFT);
            ...
            String bgImagePath = "C:\\Users\\Souid\\Desktop\\javafxapp\\ProjetPiDD\\src\\Evenements\\Gui\\bg.png";
...
canvas.addImage(bgImage);
          ...
          // Add the logo to the document
            String logoImagePath = "C:\\Users\\Souid\\Desktop\\javafxapp\\ProjetPiDD\\src\\Evenements\\Gui\\logoequipe.png";
...
document.add(scan);
    ...
    document.add(footer);
String imagePath1 = "C:\\Users\\Souid\\Desktop\\javafxapp\\ProjetPiDD\\src\\Evenements\\Gui\\Signature.png";
