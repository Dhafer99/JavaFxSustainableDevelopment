...
public class DonFController implements Initializable {
  ...
  private ObservableList<entities.Annonces> originalDonList;
private ObservableList<entities.Annonces> filteredDonList;
    @FXML
    ...
    try {
        ...
        int count = 0;
        for (entities.Annonces d : personnes) {
            if (count >= 3) {
                break;
            }
...
private void filterDonList(String query) throws SQLException {
    ...
    for (entities.Annonces d : originalDonList) {
...
private void updateUI(ObservableList<entities.Annonces> donList) throws SQLException, IOException {
    // Clear the container before populating it
    favoriteContainer.getChildren().clear();
  
    for (entities.Annonces d : donList) {
        FXMLLoader fxmlLoader = new FXMLLoader();
        fxmlLoader.setLocation(getClass().getResource("song.fxml"));
